...
import time
import statistics
from basketball.constants import TEAM_TO_TEAM_ABBR, TEAM_SETS , NBA_TEAM
    

    

def pageData(team,season_end_year):
    r = get(
        f'https://www.basketball-reference.com/teams/{team}/{season_end_year}.html')
    print(r.status_code)
    
    sportsLines = get(f'https://sports.yahoo.com/nba/odds')
    df = {}
    stats = []
    for i in NBA_TEAM:
         if NBA_TEAM[i] == team:
    ...
    if r.status_code and sportsLines.status_code== 200:
        soup = BeautifulSoup(r.content, 'html.parser')
        lines = BeautifulSoup(sportsLines.content, 'html.parser')
        perGame = soup.find('table',attrs={'id':'per_game'})
        cols = perGame.find('thead')
        cols = cols.find('tr')
        perSTATS = perGame.find('tbody')
        perSTATS = perSTATS.find_all('tr')
